...
namespace DEAXODraw.Commands
{
    ...
    public class AutoSectionCommand : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            try
            {
                ...
                try
                {
                    ...
                    var references = uidoc.Selection.PickObjects(ObjectType.Element, selectionFilter,
                        "Select elements for section views");
            ...
            {
                message = $"Error in AutoSection command: {ex.Message}";
                return Result.Failed;
            }
        }
        ...
        private Result ProcessElements(Document doc, List<Element> elements, View viewTemplate)
        {
            try
            {
                using (Transaction trans = new Transaction(doc, "DEAXO AutoSection - Create Section Views"))
                {
                    trans.Start();
            ...
            {
                TaskDialog.Show("Error", $"Failed to create section views: {ex.Message}");
                return Result.Failed;
            }
        }
        ...
        private SectionResult ProcessSingleElement(Document doc, Element element, View viewTemplate)
        {
            try
            {
                ...
                // Create only cross-section view
                ViewSection sectionView = generator.CreateCrossSectionOnly(viewNameBase);
                if (sectionView == null)
                {
                    return null;
                }
                ...
                ViewSheet newSheet = CreateSheetWithView(doc, sectionView, typeName, categoryName, element.Id);
            ...
            string categoryName, ElementId elementId)
        {
            try
            {
                ...
                string sheetName = $"{categoryName} - Section (DEAXO GmbH)";
        ...
        private void ShowResults(List<SectionResult> results, int successCount, int failCount)
        {
            try
            {
                string message = $"AutoSection Results:\n\n";
                message += $"✓ Successfully created: {successCount} section views\n";
                ...
                TaskDialog dialog = new TaskDialog("DEAXO AutoSection Results");
                dialog.MainInstruction = "Section Views Created Successfully";
                dialog.MainContent = message;
                dialog.Show();
            }
            catch (Exception ex)
            {
                TaskDialog.Show("Results", $"Created {successCount} section views. Failed: {failCount}");
            }
        }
